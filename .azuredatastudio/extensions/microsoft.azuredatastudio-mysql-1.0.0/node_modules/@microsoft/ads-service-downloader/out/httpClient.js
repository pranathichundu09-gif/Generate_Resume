...
class HttpClient {
    ...
    getHttpClientOptions(url, proxy, strictSSL) {
        const agent = (0, proxy_1.getProxyAgent)(url, proxy, strictSSL);
        let options = {
            host: url.hostname,
            path: url.path,
            agent: agent,
    ...
    handleDataReceivedEvent(progress, data) {
        ...
        if (progress.packageSize > 0) {
            ...
            if (newPercentage !== progress.downloadPercentage) {
                this.eventEmitter.emit("download_progress" /* Events.DOWNLOAD_PROGRESS */, newPercentage);
    ...
    handleSuccessfulResponse(pkg, response) {
        return new Promise((resolve, reject) => {
            ...
            this.eventEmitter.emit("download_start" /* Events.DOWNLOAD_START */, pkg.url, progress.packageSize);
            response.on('data', data => {
                this.handleDataReceivedEvent(progress, data);
            });
            let tmpFile = fs.createWriteStream(undefined, { fd: pkg.tmpFile.fd });
            response.on('end', () => {
                this.eventEmitter.emit("download_end" /* Events.DOWNLOAD_END */);
                resolve();
            });
